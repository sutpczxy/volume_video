...
if __name__ == '__main__':
    ...
    video_file = "rtsp://admin:sutpc654321@10.10.150.100:554/h264/ch1/sub/av_stream"
    #video_file = u"F:\\工作\\2017年下半年工作\\智慧路灯图像识别\\程序\\flow_calculate\\智慧路灯测试数据集\\1223_daybreak_3.mp4"
    ...
    while True:
        try:
            ...
            if former_period == 288:
                db_time = period_change()

            time_count +=1
            if time_count == 1500:
                print period,db_time
            #print period,db_time
            if period != former_period and former_period!=-1:
                #time,period,lane,flow,speed,car_flow,bus_flow,density,occ,state
                uid = str(uuid.uuid3(uuid.NAMESPACE_DNS,uuid_time))
                for lane in range(1,5):
                    try:
                        speed = int(speed_count[lane]/vehicle_count[lane])
                    except:
                        speed = 0

                    #flow_dif = vehicle_count[lane]-(small_vehicle_count[lane]+big_vehicle_count[lane])
                    #if flow_dif!=0:
                    #    small_vehicle_count[lane] = small_vehicle_count[lane]+flow_dif


                    sql = "insert into video_vehicle_detect VALUES ('%s','%s',%s,%s,%s,%s,%s," \
                      "%s,%s,%s,%s,%s)" % (uid+str(lane), 'VD000001', int(db_time),former_period, lane,vehicle_count[lane],
                                              speed, small_vehicle_count[lane]
                                            ,big_vehicle_count[lane],-1,-1, -1)
                    cursor.execute(sql)
                    conn.commit()
                ...
                cv2.line(output_frame, (int(output_x_ratio*area_x_left), int(output_y_ratio*area_up_y)), (int(output_x_ratio*area_x_right),
                ...
                for win_index, value in enumerate(active_window):
                    ...
                    cv2.line(output_frame, (int(value.x1*output_x_ratio), int(area_up_y*output_y_ratio)), (int(value.x1*output_x_ratio)
                ...
                cv2.imwrite("image\\"+str(int(period))+"_"+str(min)+"_"+str(second)+"_"+str(img_count)+".jpg",output_frame)
